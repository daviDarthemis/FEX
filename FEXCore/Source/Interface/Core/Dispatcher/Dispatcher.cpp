...
void Dispatcher::EmitDispatcher() {
  ...
  if (std::popcount(VirtualMemorySize) == 1) {
    ...
    and_(ARMEmitter::Size::i64Bit, TMP4, RipReg.R(), ARMEmitter::Reg::r3);
  }
...

  {
    ...
    if (!PERSIST_ABIARGS) {
      mov(PERSIST1, ARMEmitter::XReg::x0);
    }
  ...
  // Need to create the block
  {
    ...
    if (!PERSIST_ABIARGS) {
      mov(ARMEmitter::XReg::x2, PERSIST3);
    }
  ...
  auto EmitLongALUOpHandler = [&](auto R, auto Offset) {
    ...
    if (!PERSIST_ABIARGS) {
      mov(ARMEmitter::XReg::x0, PERSIST1);
      mov(ARMEmitter::XReg::x1, PERSIST2);
      mov(ARMEmitter::XReg::x2, PERSIST3);
    }
