...
namespace Context {
  ...
  bool HandleUnalignedAccess(ARM64_NT_CONTEXT *Context) {
    ...
    const auto Result = FEXCore::ArchHelpers::Arm64::HandleUnalignedAccess(false, Context->Pc, &Context->X0);
    if (!Result.first) {
      return false;
    }
...
void NotifyMemoryAlloc(void *Address, SIZE_T Size, ULONG Type, ULONG Prot) {
  if (auto *Thread = GetCPUArea().ThreadState()) {
    InvalidationTracker.HandleMemoryProtectionNotification(Thread, 
...
void NotifyMemoryFree(void *Address, SIZE_T Size, ULONG FreeType) {
  if (auto *Thread = GetCPUArea().ThreadState()) {
    if (!Size) {
      InvalidationTracker.InvalidateContainingSection(Thread, reinterpret_cast<uint64_t>(Address), true);
    } else if (FreeType & MEM_DECOMMIT) {
      InvalidationTracker.InvalidateAlignedInterval(Thread, reinterpret_cast<uint64_t>(Address),
...
void NotifyMemoryProtect(void *Address, SIZE_T Size, ULONG NewProt) {
  if (auto *Thread = GetCPUArea().ThreadState()) {
    InvalidationTracker.HandleMemoryProtectionNotification(Thread, reinterpret_cast<uint64_t>(Address),
